...
void VKPipeline::createPipelineCache()
{
	// 파이프라인 캐시는 구축된 파이프라인들을 저장하는 풀이다. 이를 통해 응용 프로그램은 파이프라인
	// 생성 오버헤드를 줄일 수 있다. 이 오베헤드에는 파이프라인 실행 간의 오버헤드와 후속 응용 프로그램
	// 실행 간의 오버헤드 두가지가 있다..
	// 파이프라인에서의 오버헤드) 파이프라인 구축 재사용. 중복되는 드로잉 개체를 만드는 데 매우 유용
	// 응용 프로그램에서의 오버헤드) 다수의 파이프라인 스테이트 개체 처리. Vulkan내부의 파이프라인 캐시는 
	// 순차화해 효과적으로 다시 사용할  수 있ㄷ..
...
bool VKPipeline::createPipeline(VKDrawable* drawableObj, VkPipeline* pipeline, VKShader* shaderObj, VkBool32 includeDepth, VkBool32 includeVi)
{
	// Initialize the dynamic states, initially it’s empty
	VkDynamicState dynamicStateEnables[2]; //view + scis
